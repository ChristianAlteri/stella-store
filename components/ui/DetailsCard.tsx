...
const DetailsCard: React.FC<DetailsCardProps> = ({
    ...
    return ( 
        <div className="flex flex-col m-5">
            <div className="m-2">
                <h1 className="text-sm hover:underline hover:cursor-pointer">{data?.designer?.name.toUpperCase()}</h1>
                <h1 className="text-xs p-1">{data?.name}</h1>
            </div>
            <div className="m-2">
                    <div className="flex flex-row gap-1">
                        <h1 className="text-xs">Sold By </h1>
                        <h1 className="text-xs hover:underline hover:cursor-pointer"> {data?.seller?.instagramHandle.split("@")[1].toUpperCase()}</h1>
                    </div>
                <div className="p-1">
