...
const MiniProductCarousel: React.FC<MiniProductCarouselProps> = ({ data, miniProductTitle, miniProductRoute }) => {
  ...
  const handleProductClick = (product: Product) => {
    router.push(
      `/product/${product?.category?.id}/${product?.designer?.id}/${product?.id}/${product?.seller?.id}`
    );
  };
            ...
            <div key={i} className={`relative h-full w-full transition-opacity duration-200 ease-in-out ${product?.isCharity ? "blur-xl" : ""}`}>
              ...
              ? (
                <ReactPlayer
                  ...
                  playing={true}
                  muted={true}
                  alt={`${product.name} from ${product.seller?.instagramHandle} by ${product.designer?.name} in size ${product.size?.name} for £${product.ourPrice} (RRP £${product.retailPrice})`}
